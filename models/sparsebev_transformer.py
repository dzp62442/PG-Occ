...
class SparseBEVSelfAttention(BaseModule):
    def __init__(self, embed_dims=256, num_heads=8, dropout=0.1, pc_range=[], scale_adaptive=True, past_queries=None):
        super().__init__()
        self.pc_range = pc_range
    ...
    def inner_forward(self, query_bbox, query_feat, pre_attn_mask=None):
        ...
        if self.gen_tau is not None:
            ...
            current_queries = attn_mask.shape[2]
...
class SparseBEVSampling(BaseModule):
    def __init__(self, embed_dims=256, num_frames=4, num_groups=4, num_points=8, num_levels=4, pc_range=[], init_cfg=None, use_anisotropy_encoding=True):
    ...
    def forward(self, query_bbox, query_feat, mlvl_feats, anisotropy_info, img_metas):
        if self.training and query_feat.requires_grad:
            return cp(self.inner_forward, query_bbox, query_feat, mlvl_feats, anisotropy_info, img_metas, use_reentrant=False)
        else:
