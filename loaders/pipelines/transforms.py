...
class GlobalRotScaleTransImage(object):
    ...
    def rotate_z(self, results, rot_angle):
        ...
        for view in range(len(results['ego2img'])):
            results['ego2img'][view] = (torch.tensor(results['ego2img'][view]).float() @ rot_mat_inv).numpy()
    ...
    def scale_xyz(self, results, scale_ratio):
        ...
        for view in range(len(results['ego2img'])):
            results['ego2img'][view] = (torch.tensor(results['ego2img'][view]).float() @ scale_mat_inv).numpy()
